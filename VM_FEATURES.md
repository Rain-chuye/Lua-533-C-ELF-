...
## 7. 原生二进制保护模块 (`vmp` 模块)
该模块直接集成在 Lua 解释器中，允许通过 Lua 脚本对 `.so` 和 ELF 文件进行商业级保护：
- **`vmp.protect(input, output)`**:
    - **全架构支持**: 自动识别并支持 ELF32 和 ELF64 (ARM/x86)。
    - **原生字符串加密**: 自动扫描并对 `.rodata` 和 `.data` 段中的字符串进行 XOR 加密。
    - **符号表混淆**: 自动混淆 `.symtab` 和 `.comment` 等段名称。
    - **入口点加固**: 对可执行文件的入口点进行位变换。
